...
# какое самое маленькое возможное число можно получить путём
# постоянного деления числа на 2.
# Изначально число равно единице.
# Также, помимо самого числа,
...
# Для начала программисты решили проверить,
# как у компьютера обстоят дела с вычислениями вещественных чисел.
# Разработчики компьютера предупредили,
...
# Также есть переменная Х,
# которая изначально равна единице.
...
# 1) в целом самый простой контроль ввода - это функция float(),
# которая применяется к input(). Это уже защитит нас от ввода некорректных
# символов
# 2) При желании мы можем усложить контроль ввода
# и добавить проверки мантиссы и порядка:

while True:
    # Если мы хотим проверить мантиссу
    # - то нам нужно работать с числом как со строкой,
    # поэтому float к input добавлять пока не будем
    ...
    for cipher in delta:
        if cipher == 'e':
            break
        mantissa += cipher
    # получив мантиссу - мы можем проверить,
    # что она удовлетворяет условию (равна числу от 1 до 9)
    # так же мы сразу можем добавить проверку порядка
    # - если порядок отрицательный,
    # то введенное число будет меньше 1, это мы и проверим:
    if 1 <= float(mantissa) <= 9 and float(delta) < 1:
        print('Число введено верно!')
        delta = float(delta)
        break
    else:
        print("Число введено с ошибкой, "
              "мантисса всегда должна быть равна числу от 1 до 9, "
...
# Для наглядности учитель написал программу,
